 ...
 * @author cnsaeman
 */
public class PluginUpdateCompleteRecent extends Thread {

    public static final HashMap<String,String> metaData=new HashMap<String,String>() {
        {
            put("title"             ,"Update Recent Publication Record");
            put("author"            ,"Christian Saemann");
            put("version"           ,"1.2");
            put("help"              ,"This plugin updates records of papers from the last two years using Inspire.net.");
    ...
    public void run() {
        ...
        String year2=String.valueOf(calendar.get(java.util.Calendar.YEAR)-1);
        if (key != null) {
            if (date != null) {
                if (date.startsWith(year1) || date.startsWith(year2)) {
                    if (((BTR2.get("journal")==null) || (BTR2.get("journal").equals(""))) &&
                        ...
                        if (bib.length() > 5) {
                            if (!comp(bib, Information.get("bibtex"))) {
                                ...
                                BTR2 = new celsius.BibTeXRecord(Information.get("bibtex"));
